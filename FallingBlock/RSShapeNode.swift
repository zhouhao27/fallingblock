...
public class RSShapeNode : SKNode {
  ...
  public convenience init( path: CGPath, centered: Bool = false) {
    var thePath = path
    
    if centered {
      let frame = path.boundingBox
      var transform = CGAffineTransform(translationX: -frame.origin.x-frame.size.width/2, y: -frame.origin.y-frame.size.height/2)
      thePath = path.copy(using: &transform)!
    }
  ...
  public convenience init(rect: CGRect) {
    self.init(path: CGPath(rect: rect, transform: nil))
  }
  ...
  public convenience init(rect: CGRect, cornerRadius radius: CGFloat) {
    self.init(path: CGPath(roundedRect: rect, cornerWidth: radius, cornerHeight: radius, transform: nil))
  }
  ...
  public convenience init(ellipseInRect rect: CGRect) {
    self.init(path: CGPath(ellipseIn: rect, transform: nil))
  }
  ...
  public convenience init(points: [CGPoint], closed: Bool = true) {
    if points.isEmpty { self.init() }
      ...
      if closed && points.count > 2 { path.closeSubpath() }
  ...
  public convenience init(controlPoints points: [CGPoint], closed: Bool = true) {
    if points.isEmpty { self.init() }
      ...
      for (index, point) in points.enumerated() {
        let mid = midBetweenPoint(point1: prevPoint, point2: point)
